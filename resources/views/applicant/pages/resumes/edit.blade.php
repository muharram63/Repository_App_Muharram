    ...
    <main class="main">
        ...
        <div class="topbar">
            <div>
                ...
                <p id="pageSubtitle">{{ __('Измените данные резюме и сохраните изменения.') }}</p>
            </div>
            ...
            <div class="">
                ...
                <div>
                    <div class="form-card" style="margin-left: 17vh;">
                        <form action="{{route('applicant.resume.update', $resume)}}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="card-head">

                                <p class="eyebrow">{{ __('Редактирование') }}</p>
                                <h1>{{ __('Изменение резюме') }}</h1>
                                <p>{{ __('Обновите данные резюме — изменения сразу увидят работодатели.') }}</p>
                            </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="text" placeholder="{{ __('Например: Backend-разработчик (Laravel)') }}"
                                           name="profession" value="{{old('profession', $resume->profession)}}">
                                </div>

                                <div class="field-group">
                                    <label>{{ __('Опыт вашей работы?') }}</label>
                                    <input type="number" name="experience_years" value="{{old('experience_years', $resume->experience_years)}}">
                                </div>

                                <div class="field-group split">
                                    <div>
                                        <label>{{ __('Желаемая должность') }}</label>
                                        <input type="text" name="desired_position" value="{{old('desired_position', $resume->desired_position)}}">
                                    </div>
                                    <div>
                                        <label>{{ __('Желаемая зарплата') }}</label>
                                        <input type="number" name="desired_salary" value="{{old('desired_salary', $resume->desired_salary)}}">
                                    </div>

                                    <div>
                                        <label>{{ __('Ссылка вашего Github или Linked') }}</label>
                                        <input type="text" name="url_website" value="{{old('url_website', $resume->url_website)}}">
                                    </div>
                                ...
                                <div class="field-group">
                                    ...
                                    <input type="text" placeholder="{{ __('Место где вы работали.') }}" name="place_work"
                                           value="{{old('place_work', $resume->place_work)}}">
                                </div>
                                <div class="field-group">
                                    <label>{{ __('О себе') }} <span class="opt">{{ __('(необязательно)') }}</span></label>
                                    <textarea name="description"
                                              placeholder="{{ __('Коротко расскажите о своём опыте, проектах и целях.') }}">{{old('description', $resume->description)}}</textarea>
                                </div>
                                <div class="field-group">
                                    ...
                                    @if($resume->documents)
                                        <p style="margin:0 0 8px; font-size:13px;">
                                            {{ __('Текущий файл:') }}
                                            <a href="{{ route('public.files.resume', $resume) }}" target="_blank">{{ basename($resume->documents) }}</a>
                                            <span class="opt">{{ __('— загрузите новый, чтобы заменить') }}</span>
                                        </p>
                                    @endif
                                        ...
                                        @foreach(\App\Models\Resume::STYLES as $key => $title)
                                            <label class="skin-option">
                                                <input type="radio" name="style" value="{{ $key }}"
                                                       @checked(old('style', $resume->style ?: 'classic') === $key)>
                            ...
                            <div class="card-foot">
                                ...
                                <button type="submit" class="submit-btn">{{ __('Сохранить изменения') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
