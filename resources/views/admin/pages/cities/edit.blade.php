...
@section('main')
    ...
    <div class="cc-wrap">
        ...
        <div class="cc-topbar">
            <h1 class="cc-title">Изменить Страну</h1>
        </div>

        <form action="{{route('superadmin.cities.update',$city)}}" method="post" class="cc-card" >
            @csrf
            @method('PUT')
            <div class="cc-field">
                <label for="cc-name">Страна</label>
                <input type="text" id="cc-name" placeholder="Например: Таджикистан" name="country" value="{{old('country',$city->country)}}">
                <div class="cc-hint">Отображается пользователям в списке Соискателей и Работаделей</div>
            </div>
            <div class="cc-field">
                <label for="cc-name"></label>
                <input type="text" id="cc-name" placeholder="Например: Согдийская область,город Худжанд" name="region" value="{{old('region',$city->region)}}">
            </div>
