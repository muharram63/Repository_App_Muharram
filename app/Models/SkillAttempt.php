...
class SkillAttempt extends Model
{
    ...
    /**
     * Сколько минут даётся на один вопрос. Раньше на всё задание отводилось
     * ровно 15 минут, но длину задания теперь выбирает кандидат — значит и
     * время должно считаться от неё, иначе полное задание не успеть.
     */
    public const MINUTES_PER_QUESTION = 3;

    /** Срок для задания из указанного числа вопросов. */
    public static function minutesFor(int $questions): int
    {
        return max(1, $questions) * self::MINUTES_PER_QUESTION;
    }
